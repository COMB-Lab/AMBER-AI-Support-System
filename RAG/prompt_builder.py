...
def build_context(chunks: List[Dict], max_chars: int = 9000) -> str:
    """
    Mirrors notebook build_context():
    - Adds headers for traceability INSIDE context (model must not repeat them)
    - Stops at max_chars
    """
    parts: list[str] = []
    used = 0
    ...
    for i, ch in enumerate(chunks, 1):
        subject = ch.get("subject", "(no subject)")
        author = ch.get("author", "Unknown")
        sim = ch.get("similarity", 0.0)

        header = f"[CITE {i}] {subject} | {author} (similarity={float(sim):.3f})"
...
def build_prompt(question: str, context: str) -> list:
    """
    Mirrors notebook build_prompt():
    Returns chat-style messages for the Ollama interface.
    """
    return [
