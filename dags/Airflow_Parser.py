...
def parse(input_path: pathlib.Path, output_path: pathlib.Path):
    ...
    if author_tag:
        ...
        if data["date_raw"]:
            ...
            # Use the email utility to handle common variations in date formats
            dt_object = parsedate_to_datetime(data["date_raw"])
            ...
            data["date_utc"] = dt_utc.strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    if subject_tag:
        ...
        if len(sParts) > 1:
            ...
            # Handle cases where '[AMBER]' is not in the subject
            # Here, we'll use the whole subject as the thread_id
            data["thread_id"] = thread_string
